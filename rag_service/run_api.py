...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "app.main:app",
        # 127.0.0.1 — RAG-сервис вызывается только локальным ботом с той же машины.
        # 0.0.0.0 (слушать на всех интерфейсах) опасно: если на сервере нет
        # настроенного firewall, /search и /index становятся доступны из интернета.
        # Если когда-то понадобится доступ снаружи — выставляйте 0.0.0.0 ТОЛЬКО
        # вместе с firewall, ограничивающим порт 8001.
        host="127.0.0.1",
        port=8001,
        ...
        workers=1,  # 1 воркер для экономии RAM (сервер 1 ГБ)
    )
